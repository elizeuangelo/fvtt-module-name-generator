...
export default function contextMenu(html: JQueryStatic, entryOptions: entryOption[]) {
	entryOptions.push({
		name: 'Rename Character',
		icon: '<i class="fas fa-users-cog"></i>',
		callback: (li) => {
			const actor = game.actors.get(li[0].dataset.documentId ?? '') as any;
			...
			if (data.name1) data.locks.name1 = false;
			if (data.name2) data.locks.name2 = false;
			if (data.name3) data.locks.name3 = false;

			if (data.portrait) data.locks.portrait = true;
			if (actor.folder) data.folder = actor.folder.id;
