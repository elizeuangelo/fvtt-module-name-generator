...
import { log, error, pick, recursive_menu, seeder } from './utils.js';
export class DB {
    name;
    loaded = false;
    data = [];
...
class NameGenerator {
    ...
    label;
    get;
    constructor(label = '', data, fn = genericNameGen) {
        this._source = data;
        this.label = label;
        this.get = fn;
    }
...
async function genericNameGen(option = 0, prefix = 'The first name means') {
    ...
    let meaning = '';
    if (meaning_index)
        meaning = item[meaning_index];
    if (meaning)
        meaning = prefix + ' "' + meaning + '".';
    return [item[0], meaning];
}
async function genericNameGen2() {
    const db = this._source;
    if (!(await loadDB(db)))
        return ['', ''];
...
async function dwarvenSurnameGen() {
    const db = this._source;
    if (!(await loadDB(db)))
        return ['', ''];
...
async function loadDB(db) {
    if ('loaded' in db && !db.loaded && !(await db.load()))
        return false;
    return true;
}
const DBs = {
...
const generators = {
    ...
    dwarven_names: new NameGenerator('Name', DBs.dwarven_names, async function () {
        const db = this._source;
        let item;
        if (!(await loadDB(db)))
            return ['', ''];
    ...
    fr_dwarven_surnames: new NameGenerator('Family', [], async function () {
        if (seeder.rnd() > 0.1) {
            return [(await generators.dwarven_surnames.get())[0] + ', of the', ''];
        }
        return ['', ''];
    }),
    fr_dwarven_clans: new NameGenerator('Clan', DBs.fr_dwarven_clans, async function () {
        const item = await genericNameGen2.apply(this, []);
        if (item[1])
            item[1] = `This is a dwarven clan from ${item[1]}ern Faerûn.`;
        return item;
    }),
    duergar_names: new NameGenerator('Name', DBs.duergar_names, async function () {
        const db = this._source;
        let item;
        if (!(await loadDB(db)))
            return ['', ''];
    ...
    elven_houses: new NameGenerator('House', DBs.elven_surnames, async function () {
        const name = await genericNameGen2.apply(this, []);
        if (name[1] && name[1].split(' ').length === 1)
            name[1] = 'The house name means "' + name[1] + '".';
        return name;
    }),
